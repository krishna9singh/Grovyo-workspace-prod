...
function page() {
  ...
  const topicskoEditkro = () => {
    if (type === "Free") {
      setArrtopic([...arrtopic, { name: topicTitle, msg: message, ty: "free", pri: "no price" }])
    }
    else {
      setArrtopic([...arrtopic, { name: topicTitle, msg: message, ty: "Paid", pri: price }])
    }
    setTopics(false)
    setmessage("")
    setTopicTitle("")
  ...
  const fetc = async () => {
    const i = await sessionStorage.getItem("id");
    setId(i);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // formDataToSend.append("topic", arrtopic.filter((d) => d.name).map((d, i) => { return d.name }));
    // formDataToSend.append("type", arrtopic.filter((d) => d.ty).map((d, i) => { return d.ty }));
    // formDataToSend.append("price", arrtopic.filter((d) => d.pri).map((d, i) => { return d.pri }));
    // formDataToSend.append("message", arrtopic.filter((d) => d.msg).map((d, i) => { return d.msg }));

    arrtopic.forEach((e) => {
      formDataToSend.append("topic", JSON.stringify(e));
    })
  ...
  console.log(message, type, price, title);
  return (
    <div>
      {/* popup 1 */}
      <div
      ...
      <div className="overflow-auto overflow-x-hidden scrollbar-hide h-full ">
        <div className=" pt-1 pb-20">
          ...
          <div className="bg-white w-full h-12 rounded-t-2xl shadow-md sm:hidden fixed bottom-0 z-30 flex justify-between items-center px-6">
            ...
            <div
              onClick={handleSubmit}
              className="px-10 py-2 font-semibold ring-2 ring-[#002eff] bg-[#002EFF] text-white rounded-2xl"
            >
              save
            </div>
          </div>
                    ...
                    .filter((d) => d.name).map((d, i) => (
                      <div key={i} className="flex justify-between px-4 bg-[#F4F5F7] rounded-xl py-2 items-center mt-2">
                        ...
                        <div className="flex justify-between gap-2">
                          <AiOutlineEdit onClick={() => setTopics(true)} className="w-7 h-7 text-[#4362ec] rounded-2xl" />
                          <RiDeleteBin6Line onClick={() => setTopicTitle
                            ("")} className="py-2 px-2 bg-red-500 rounded-2xl text-white w-8 h-8" />
                        </div>
                      </div>
                    ))}
                </div>
